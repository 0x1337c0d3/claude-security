...
make_findings_json() {
    ...
    i=0; while [[ $i -lt $critical ]]; do
        ...
        findings+=$(printf '{"id":"SENTINEL-%03d","severity":"CRITICAL","title":"Critical %d","source_tool":"test","file":"c%d.js","line":%d,"status":"confirmed"}' "$id" "$id" "$id" "$id")
        id=$((id + 1)); i=$((i + 1))
    done
    i=0; while [[ $i -lt $high ]]; do
        [[ "$first" == "true" ]] && first=false || findings+=","
        findings+=$(printf '{"id":"SENTINEL-%03d","severity":"HIGH","title":"High %d","source_tool":"test","file":"h%d.js","line":%d,"status":"confirmed"}' "$id" "$id" "$id" "$id")
        id=$((id + 1)); i=$((i + 1))
    done
    i=0; while [[ $i -lt $medium ]]; do
        [[ "$first" == "true" ]] && first=false || findings+=","
        findings+=$(printf '{"id":"SENTINEL-%03d","severity":"MEDIUM","title":"Medium %d","source_tool":"test","file":"m%d.js","line":%d,"status":"confirmed"}' "$id" "$id" "$id" "$id")
        id=$((id + 1)); i=$((i + 1))
    done
    i=0; while [[ $i -lt $low ]]; do
        [[ "$first" == "true" ]] && first=false || findings+=","
        findings+=$(printf '{"id":"SENTINEL-%03d","severity":"LOW","title":"Low %d","source_tool":"test","file":"l%d.js","line":%d,"status":"confirmed"}' "$id" "$id" "$id" "$id")
        id=$((id + 1)); i=$((i + 1))
    done
