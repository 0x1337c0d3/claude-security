...
name: audit
description: >
  Intelligence-driven security analysis — the reasoning layer that complements Sentinel's
  ...
  or pastes Sentinel's consolidated.json for deeper analysis. Also trigger on any security
  question after `/sentinel:sentinel` has run. Works without any tools installed — pure
  Claude intelligence. Part of the Sentinel plugin (github.com/alissonlinneker/sentinel-claude-skill).
---
...
- "SENTINEL-042 appears to be a false positive: `escape()` at line 18 sanitizes before the sink at line 34."
