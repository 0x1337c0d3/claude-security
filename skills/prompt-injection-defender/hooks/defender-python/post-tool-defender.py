...
def log_data(
    ...
    try:
        ...
        log_path = log_dir / "post_tool_defender.json"
        ...
        entry = dict(input_data)
        if detections:
            entry["detections"] = [
                {"category": c, "pattern": p, "reason": r, "severity": s}
                for c, p, r, s in detections
            ]
...
def main() -> None:
    ...
    log_data(input_data, detections)
