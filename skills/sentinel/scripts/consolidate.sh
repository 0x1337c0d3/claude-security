...
# Deduplicates findings, assigns SENTINEL-XXX IDs, sorts by severity,
...
echo "$MERGED" | jq \
    ...
    '
    # Assign sequential SENTINEL-XXX IDs
    . as $findings |
    {
        "findings": [
            $findings | to_entries[] | .value + {
                "id": ("SENTINEL-" + ((.key + 1) | tostring | if length < 3 then ("000" + .)[-3:] else . end))
            }
        ],
        "metadata": {
            "scan_date": $scan_date,
