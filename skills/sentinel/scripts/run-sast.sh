...
# Always start with the community ruleset, then layer custom rules on top.
SEMGREP_ARGS=(--config auto)
echo "Using semgrep auto config (community rules)" >&2

# Layer language-specific custom rules if present
for candidate in "$RULES_DIR/${LANGUAGE}.yaml" "$RULES_DIR/${LANGUAGE}.yml"; do
    if [[ -f "$candidate" ]]; then
        SEMGREP_ARGS+=(--config "$candidate")
        echo "Layering custom rules: $candidate" >&2
        break
    fi
done

# Layer common rules (cross-language) if present
for common in "$RULES_DIR/common.yaml" "$RULES_DIR/common.yml"; do
    if [[ -f "$common" ]]; then
        SEMGREP_ARGS+=(--config "$common")
        echo "Layering common rules: $common" >&2
        break
    fi
done
...
TEMP_OUTPUT="$(mktemp /tmp/sentinel-sast-XXXXXX.json)"
